...
%that making the textual description more compatible with
...
Note that we cannot rely on descriptions of particular species due to the zero-shot setup. We must do with descriptions of objects in the {\em general domain} %that can be be obtained for the {\em general class} 
...
%similarity  to the representation of the text-only if over 15\% of the texts from test-set are clustered with at least one text from train-set we concatenate the encoded cluster label to the document representation.
...
%For comparison, \citet{reed2016learning} showed that his model relies on at least 512 captions \textit{per class} to achieve the same accuracy as using all the captions available per class, during test-time.
